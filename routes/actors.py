...
def create_actor(
    ...
    if payload.get("user_type") != "admin":
        ...
        return db_actors.create_actor(db, request)


# Get All Actors
@router.get("/", response_model=List[ActorDisplay])
def read_all_actors(db: Session = Depends(get_db)):
    return db_actors.get_all_actors(db)


# Get Actor By Id
@router.get("/{actor_id}", response_model=ActorDisplay)
    ...
    if payload.get("user_type") == "admin":
        ...
        if actor is None:
            ...
            if db_actors.check_actors_in_movie(db, actor_id):
                ...
                raise HTTPException(
                    status_code=409,
                    detail="Actor is associated with a movie. Cannot delete.",
                )
